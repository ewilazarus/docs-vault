---
description: "How this vault is organised, and the rules that bind work on this project."
---
<!--
The docs-vault skill fixes only the journal and its #decision and #todo tags. Everything
else is defined here. Delete the sections you don't need, and this comment too.
-->

## Layout

Which folders exist, and what goes in each one.

## Notes

How notes are named, which frontmatter they carry, and which templates to start from.

## Tags and callouts

Any tags or callout types beyond the journal's `#decision` and `#todo`, and where they go.

## Rules

- **State each rule as an instruction.** Then give the reason, in the same sentence or the
  next one.
